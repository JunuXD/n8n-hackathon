...
export async function postOcrMenuRecipe(
  file: File,
  storeId: number
): Promise<any> {
  try {
    ...
    formData.append("store_id", storeId.toString()); // 파일 추가

    const response = await axios.post(
      "https://primary-production-b57a.up.railway.app/webhook/recipe-ocr", // API 엔드포인트
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data", // form-data 전송
